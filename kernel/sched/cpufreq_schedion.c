 ...
 *
 * Copyright (C) 2018-2020, Quantic Software
 * Author: Carlos Ayrton Lopez Arroyo <15030201@itcelaya.edu.mx>
 *
 *
 ...
 * @sg_policy: schedion policy object to compute the new frequency for.
...
#define DEFAULT_CPU7_RTG_BOOST_FREQ 1000
#define DEFAULT_CPU0_UP_RATE_LIMIT_US 20000
#define DEFAULT_CPU0_DOWN_RATE_LIMIT_US 150
#define DEFAULT_CPU4_UP_RATE_LIMIT_US 20000
#define DEFAULT_CPU4_DOWN_RATE_LIMIT_US 0
#define DEFAULT_CPU7_UP_RATE_LIMIT_US 500
#define DEFAULT_CPU7_DOWN_RATE_LIMIT_US 2000
...
static void sugov_irq_work(struct irq_work *irq_work)
{
	...
	/*
	 * For RT and deadline tasks, the schedion governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the sugov_work() function and before that
	 * the schedion governor rejects all other frequency scaling requests.
	 *
...
static struct cpufreq_governor schedion_gov;
 ...
 * This values are applied on each cluster, with this and Yaro high_cap distribution we ensure that prime core manages the big tasks
 * properly, letting the small cores idle avoiding waisting energy, since the scheduler find that small cores are bussy, distributes the load
 * to the big and prime cores, this cores are more efficient than the silver cores, idk why qualcomm added this old architecture
 *
 **/
	switch (policy->cpu) {
	default:
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SCHEDION
...
{
	return cpufreq_register_governor(&schedion_gov);
}
