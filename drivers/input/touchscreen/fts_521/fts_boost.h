...
/*
 * Copyright (C) 2020, Carlos Ayrton Lopez Arroyo <15030201@itcelaya.edu.mx>.
...
struct fts_touch_pdata{
	struct fts_touch *device;
	struct fts_touch_interface *touch_data;
	int palm_value;
	bool palm_changed;
	int psensor_value;
...
extern struct class *get_fts_touch_class(void);

extern struct device *get_fts_touch_dev(void);
