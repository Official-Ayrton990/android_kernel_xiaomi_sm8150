...
#mental note (?) review this flags changes
ifeq ($(cc-name),clang)
...
KBUILD_CFLAGS += $(call cc-option, -Wno-somet)
...
ifdef lto-flags
  ifeq ($(call cc-option, $(lto-flags)),)
	@echo Cannot $(cc-name) use CONFIG_LTO: $(lto-flags) not supported by compiler \
		>&2 && exit 1
  endif
endif
...
ifdef stackp-name
  ifeq ($(call cc-option, $(stackp-flag)),)
	@echo Cannot use CONFIG_CC_STACKPROTECTOR_$(stackp-name): \
		  $(stackp-flag) not supported by $(cc-name) $(cc-version) >&2 && exit 1
  endif
endif
